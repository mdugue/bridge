...
Pure Python, no third-party imports: the `trees` bake (trees.py) classifies
with it and `scripts/eval/kataster-eval.py` reuses it, so the analysis and the
committed artifact can never disagree about which tree is which archetype.
