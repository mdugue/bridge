...
Pure Python, no third-party imports: `extract-trees.sh` bakes with it and
...
EVERGREEN_CONIFERS = {
    "Abies", "Araucaria", "Cedrus", "Chamaecyparis", "Cupressocyparis",
    "Cupressus", "Juniperus", "Picea", "Pinus", "Pseudotsuga",
...
EVERGREEN_SPECIES = {"Viburnum rhytidophyllum", "Magnolia grandiflora",
                     "Prunus laurocerasus"}

SMALL_GENERA = {
    "Amelanchier", "Buxus", "Caragana", "Cercis", "Cornus", "Cotinus",
    "Crataegus", "Cydonia", "Euonymus", "Laburnum", "Magnolia", "Malus",
    "Mespilus", "Prunus", "Rhus", "Sambucus", "Sorbopyrus", "Sorbus",
    "Syringa", "Tamarix", "Tetradium", "Viburnum", "Parrotia",
}
SMALL_SPECIES = {
    "Acer palmatum", "Acer ginnala", "Acer tataricum", "Acer buergerianum",
    "Acer monspessulanum", "Corylus avellana", "Salix caprea",
    "Salix cinerea", "Salix viminalis", "Pyrus communis", "Pyrus pyraster",
    "Koelreuteria paniculata", "Cercidiphyllum japonicum",
}
OVAL_GENERA = {
    "Alnus", "Betula", "Carpinus", "Ginkgo", "Liquidambar", "Liriodendron",
    "Ostrya", "Populus", "Zelkova", "Pyrus",
}
OVAL_SPECIES = {
    "Corylus colurna", "Acer rubrum", "Acer freemannii", "Quercus palustris",
    "Pyrus calleryana", "Fraxinus angustifolia",
}
# Cultivar epithets (inside the quotes) that name an upright-oval or conic
# crown on a genus that is otherwise round.
OVAL_CULTIVARS = {
    "greenspire", "rancho", "roelvo", "pallida", "elsrijk", "huibers elegant",
    "queen elizabeth", "raywood", "cleveland", "emerald queen", "olmsted",
    "autum blaze", "autumn blaze", "armstrong", "red sunset", "october glory",
    "rebona", "new horizon", "regal", "lobel", "dodoens", "skyline",
    "chanticleer", "glenleven", "brouwers", "geessink",
    "sapporo gold", "summit", "cimmzam", "merkur", "szeleste", "brabant",
    "green vase",
}
    ...
    if known and is_conifer(genus):
        ...
        if arch is None:
            if genus in SMALL_GENERA or _species_match(binomial, SMALL_SPECIES):
                arch = SMALL
            elif genus in OVAL_GENERA or _species_match(binomial, OVAL_SPECIES):
                arch = OVAL
            elif genus == "Salix" and "matsudana" in binomial:
                arch = OVAL
            else:
                arch = ROUND
