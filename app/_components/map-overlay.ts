 ...
 * (riverside-layer.ts), the street lettering (name-layer.ts) — show only
 * from the air: faded in with the camera's height over the ground, from
 * nothing on foot to full once the view is a map. One shared uniform the
 * render loop sets (create-app.ts), like the fountains' clock.
 */
