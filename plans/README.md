...
| 004 | Preprocess the DGM into a 512² heightfield at build time (12.6 MB → ≈0.75 MB, no in-browser GeoTIFF decode) | P1 | M | — | DONE — `bun run verify` (115 unit tests), `bun run build` and `bun run test:e2e` (9/9) green. Ported onto the post-#16 tree, which changes the shape of the win: the block is **four** tiles, and the primary one is meshed at 1024², not 512². So the grid size is part of the file name (`dgm1_<tile>.heightfield-<n>.{json,f32}`), the primary bakes at 1024² and the neighbours at 512², and `lib/city/tile.ts` holds the whole tile block (list + per-tile grid size) rather than the plan's single `POC_TILE` — `prepare-data.ts` and `city-walk-client.tsx` now read the same list. Measured: **~5.2 MB gzipped over the wire (3.0 + 3×0.78) instead of ~50 MB (4×12.6)**, and four in-browser GeoTIFF decodes gone; the `.tif`/`.tfw` are no longer copied into `public/data` at all. Also done: mtime-based staleness (the size-only check is gone), NoData vertices parked at the mean valid elevation, `demTfwSrc`/`targetSize` removed from the whole chain. Not done: `geotiff` stays in `dependencies` (out of scope per the plan — a candidate for 006), and the manual `bun dev` look is open (no display here). |
| 005 | Input & collision fixes: stuck keys, diagonal speed, pinch baseline, inserted building, failure-path cleanup | P2 | M | 001 | DONE — all five steps; `bun run verify` (125 unit tests: 14 movement, 6 new touch-controls), `bun run build` and `bun run test:e2e` (9/9) green. Steps 1–3 went in verbatim, 4–5 were ported onto the rewritten `create-app.ts`. Two deviations worth knowing: the existing "analog input is clamped to the unit square" test had to change — the setter still clamps per axis, but a full diagonal now covers `WALK_SPEED`, not `WALK_SPEED·√2` — and Step 5's cleanup list grew an eighth entry beyond the plan's seven, covering the per-tile lamp/rail/wall controls and the shared lamp-light pool that #16 added (the plan's own maintenance note asks for exactly that). `dispose()` is now idempotent. The two manual checks (walk into the inserted box; one canvas after the StrictMode remount in `bun dev`) are open — no display here. |
| 006 | Scaffold cleanup, unused deps, lint config, fonts, README + AGENTS.md | P2 | M | — | DONE — Steps 1–8 + the surviving part of 9; `bun install --frozen-lockfile`, `bun run verify`, `bun run build` and `bun run test:e2e` (9/9) green. Step 5 was **recomputed** as the plan demands and the answer had moved: **23** reachable `components/ui` files, not 16 (#16's sidebar pulls in sheet/skeleton/dialog/dropdown-menu/…), and 32 unreachable ones deleted. `drawer.tsx`/`vaul` turned out to be among the unreachable — the mobile drawer is the Sidebar's off-canvas Sheet — so `vaul` went too, on top of the plan's seven: `cmdk`, `embla-carousel-react`, `input-otp`, `next-themes`, `react-resizable-panels`, `recharts`, `sonner`. `hooks/use-mobile.ts` stays (sidebar imports it), so its lint entries stay. CI had 4 `bun-version: latest`, not 5. One note on Step 4: dropping `--font-mono` does leave two HUD elements on the system mono stack — intended (one less webfont), but it is a visible change, unlike the plan's "nothing uses it". Step 9 was superseded by #16 as recorded above; only its still-true parts were added to the rewritten AGENTS.md (the `verify` command, the version-pin rule incl. `.mcp.json`, the vendored-UI rule). Dependabot PR #13 (recharts) is now moot — the dep is gone. |
