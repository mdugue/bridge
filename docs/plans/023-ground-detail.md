...
- **Status**: PARTIAL — 1–3 done, 4–8 open
...
With the NDVI raster: on classes 0 and 4, where the NDVI (sampled one mip
coarser, so the ~2 m raster reads as a soft field) passes 0.22 → 0.40 and
the ground is neither road nor pavement OSM calls sealed, the meadow colour
(and the meadow mottle) takes over; slabs are not drawn there. New look
row **`urbanGreen`** (*Stadtgrün*, default 0.6).
...
a riser plus a 0.3 m top, standing on the shaped ground. Gains contact
shadows (SSAO) and a real silhouette at the face. Keep the shader kerb for
the colour; drop its normal step where the ribbon exists. STOP if the
ribbon triangle count exceeds ~10 % of the fine terrain's.
