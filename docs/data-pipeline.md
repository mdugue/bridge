...
| 8 🧪 | `extract-trees.sh` (+ `tree_archetypes.py`) | Dresden WFS `cls:L1261` (Stadtbaumkataster), cached as `_raw/baumkataster/stadtbaum_<t>.geojson` + `.meta.json` | `dlm/trees_<t>.geojson` (points with `h`, `d`, archetype `a`, leaf type `l`, optional foliage `c`, globe `g`; `attribution` member) | `TREES_RAW_DIR` (raw cache) | The service has no paging: one BBOX request (+10 m) per tile, trusted only when its feature count equals the server's `numberMatched`. Trees are clipped half-open to the tile so a seam tree lands once. Missing height/crown imputed from this tile's genus median / archetype d:h. Only read behind `?trees=kataster`. Analysis: `scripts/eval/kataster-eval.py` |
| 8 🧪 | `extract-lowveg.sh` (+ `extract-lowveg.py` under `uv`) | `_raw/lsc/…/lsc_<t>.laz` → PDAL 0.5 m rasters in `_raw/lsc/derived/<t>/` (made when missing); Overpass (`barrier=hedge`, `natural=scrub/shrubbery/shrub`, cached in `_raw/osm/lowveg_<t>.json`); the class PNG, NDVI PNG, walls, bridges, canopy (steps 1–3, 6, 7), CityJSON, DGM | `dlm/lowveg_<t>.geojson` (hedge LineStrings `h`/`w`, shrub Points `h`/`r`, `src`, `attribution`), `dlm/canopyx_<t>.geojson` (crown peaks `h`/`r`) | `RAW_ROOT` (a worktree's `_raw`), `OVERPASS_URL` (mirror) | The only bake that needs numpy/scipy/scikit-image/shapely/rasterio — `uv run --with …` supplies them, the system Python stays Pillow-only. PDAL `writers.gdal` needs `binmode:true`, one writer per pipeline. Without the LAZ the tile falls back to OSM only. Byte-identical when re-run from the LAZ. Drawn only behind `?veg=` |
...
| 🧪 `dgm1_<t>.tin-<cm>cm.json` + `.bin.gz` (primary only, optional; read only by `?terrain=tin`) | the same DGM GeoTIFF, at its native 2000² | `bake-terrain-tin.ts` → `lib/city/terrain-tin.ts` | Delatin error-bounded TIN (`TileSpec.tinMaxError`, 0.15 m): ~153k vertices / 305k triangles, ~1.04 MB; vertices as byte-split delta planes, triangles as varints; ~1.6 s to bake |
...
bash scripts/extract-trees.sh 33412_5656       # 8  🧪 street-tree cadastre (Dresden WFS)
bash scripts/extract-lowveg.sh 33412_5656      # 8  🧪 hedges, shrubs, extra trees (LAZ + Overpass; needs 1-3, 6, 7)
