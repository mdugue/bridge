...
bun run bake --step canopy          # one step, in the order `all` runs them:
                                    ...
                                    #   riverside, names, skyview, soundmarks,
...
| `landcover.py`, `canopy.py`, `trees.py`, `ndvi.py`, `roof_colour.py`, `osm_buildings.py`, `rail.py`, `lamps.py`, `monuments.py`, `furniture.py`, `walls.py`, `stairs.py`, `surface.py`, `edges.py`, `markings.py`, `sport.py`, `tram.py`, `riverside.py`, `names.py`, `skyview.py`, `soundmarks.py`, `lowveg.py`, `cultivated.py`, `small_buildings.py` | one step each, in the order of the step table (`STEPS`; the table below) |
...
bridge decks before `furniture`, `tram`, `names`, `lowveg` and
...
| `names` | OSM `highway=*` lines with a `name` (not `service`, not `footway=sidewalk`), `place=square` / `highway=pedestrian` areas with a `name`; the tile's committed `bridge` file (the DLM names) | `dlm/names_<t>.geojson` (`k: label` lines with `name` and class `c`: main / minor / bridge / square; `k: way` lines with `name`) | Every name with a way within 50 m of the tile, and all of that name's ways in the extract, merged unclipped (the whole extract is read: GDAL's OSM driver parses the file either way), so the windows are the same on every tile — clipped to the tile + 50 m they moved with it, and 55 names were lettered on two tiles < 200 m apart (Paul-Scholze-Weg 3 m); none now (2026-09-26). Label windows: `len(name) × 4.2 m + 20 m` long, direction swinging < 20° (5 m steps), one per 450 m of line (the straightest nearest the middle of its share), none within 200 m of that name's last; written by the tile owning the window's middle, simplified 0.5 m. `main` = trunk to tertiary (and their links). Bridges: each name's largest deck on the tile; squares: each name's largest area ≥ 400 m² — both a straight label across the long axis, owned by their centre; a square's streets are not lettered again. Ways: every name's merged lines cut at the tile edge, simplified 1 m. Runs after `rail`. No `.osm.pbf`: skipped with a note, the file already there stays |
