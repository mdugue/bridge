...
| `bridge.py` | what `rail.py` measures on a deck: the roadway and the superstructure ribs in DOM1, the fairway clearance from OSM seamarks, the Wikidata match; and the ingest's Wikidata fetch ([ADR 0032](./adr/0032-bridges-measured-in-the-surface-model.md)) |
| `ingest_sn.py` | the Saxony ingest adapter (with Dresden's street-tree WFS and Wikidata's bridges) |
...
- **Wikidata, per tile:** one SPARQL query for the bridges Wikidata places
  within 500 m of the tile (class, main span, length) into
  `wikidata/bridges_<tile>.json` (`bridge.fetch_wikidata`). The bake reads
  the file, never the network.
...
| `rail` | `ver03_f`, `ver03_l`, `ver06_f`, `ver06_l`, `ver01_l`, `ver02_l`; DGM1 + DOM1 of the tile **and its neighbours**; OSM `man_made=bridge`, `seamark:type=bridge`, `railway=platform`; `wikidata/bridges_<t>.json` | `dlm/railarea_<t>.geojson` (ballast polygons), `dlm/rail_<t>.geojson` (lines with `tracks`, `electrified`), `dlm/bridge_<t>.geojson` (polygons with per-vertex `deck`, `kind`, `name`, `structure`, and `axis`, `line` (deck height per 2 m), `ribs` (`offset`, `rise` per 2 m), `fairway`, `clearance`, `depth`, `wikidata`, `span` where measured or known), `dlm/platform_<t>.geojson` | Ballast: `OBJART=42010` made valid, unioned (shapely) and clipped. Rails: heavy rail only (`SPW=1000`, trams excluded), fragments merged at 1 m. Decks: every `ver06_l` centreline (`BWF=1800`), snapped to the `ver06_f` footprint it runs on (half its length within 1 m), else buffered by kind width; every deck that touches the tile is written (the viewer draws it on the tile that owns its centre), its ground a DGM/DOM mosaic 700 m around the tile so both tiles' copies agree; deck height = the roadway DOM1 measures along the axis, held within −6/+4 m of the DGM abutment ramp and grade-limited from midspan out (8 %, rail 4 %); `kind` from the rail/road/path networks under it; ribs (only where the class stands above the deck: arch, truss, suspension, cantilever, cable-stayed): DOM1 above the deck, per half of the cross-section, opened (7 m) and closed (11 m), kept ≥ 3 m high for ≥ 25 m; `depth` from the fairway clearance over the DGM's water; `structure` from Wikidata's class when a deck matches an item (OSM `wikidata` tag, else its DLM name within 150 m), else the nearest OSM bridge ≤ 60 m. **No Basis-DLM: the step is skipped, the files already there stay. No DOM1: decks use the DGM ramp, no ribs. No `.osm.pbf`: bridges without structure or clearance, the platform file already there stays. No Wikidata file: the OSM structure.** Every other output is written, even when empty |
