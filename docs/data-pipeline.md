...
| `landcover.py`, `canopy.py`, `ndvi.py`, `roof_colour.py`, `lamps.py`, `monuments.py`, `furniture.py`, `walls.py`, `stairs.py`, `rail.py` | one step each (table below) |
...
| `furniture` | OSM points `amenity=bench/waste_basket/bicycle_parking/post_box/shelter`, `leisure=picnic_table`, `barrier=bollard`, `highway=bus_stop` / `public_transport=platform` with `shelter=yes`; bench ways; every `highway` line; the class raster, the tile's `bridge` file | `dlm/furniture_<t>.geojson` (points with `k`: bench / picnic / bin / bike / bollard / postbox / shelter; `a` the bearing it faces, degrees from north; `l` a bench way's length; `back: false`; `n` a stand's hoops) | `a`: OSM `direction` on a bench (degrees or a compass point), else towards the nearest highway line within 25 m, across it when the object stands on it (< 0.5 m); a bench way stands at its midpoint facing the path side, length clamped 1–8 m. `n` = ⌈`capacity` ÷ 2⌉, 1–12. Dropped: `bicycle_parking=wall_loops`, `amenity=shelter` other than public transport, stops without `shelter=yes`, `indoor=yes`, `location=indoor/underground`, `level` < 0, classes 5 and 8, inside a bridge deck; a shelter within 8 m of another. Only what the tile owns. No `.osm.pbf`: skipped with a note, the file already there stays |
