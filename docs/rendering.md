...
| Signs and fixtures | OSM point → advertising column (paper drum Ø 1.2 m, 2.7 m, a darker ring and dome, five pastel poster fields; `lit` ones emissive × `nightFactor`), traffic signal (3.2 m pole, dark three-lamp head facing `a`, unlit), pillar hydrant (0.8 m, red ochre), underground-hydrant sign plate on a post (at 70 %), clock on a 3.5 m post (double face) or on a facade (one face, bracket to the wall), drinking fountain (1 m bronze column + basin), bus-stop "H" sign (2.6 m pole, yellow disc, green H, timetable box); clock hands turned by the scene's time (`uClockMinutes`, on the minute), never casting | OSM | `furniture-layer.ts` (`column`, `signal`, `hydrant`, `hydrantSign`, `poleClock`, `wallClock`, `drinkingWater`, `stopSign`, `clockHands`), `pipeline/bake/furniture.py` |
...
| Street lettering | label line + `name` + class → a ribbon on the ground along it (4 m samples, +0.2 m), letters 4 m (main roads, squares 6 m, bridges 5 m) from a per-tile Canvas-2D atlas in Inter, slate ink with a pale halo; alpha 0.85 × the map fade (camera 25 → 60 m over the ground) × (minor roads: out between 200 and 250 m); unlit, never casts | OSM (+ Basis-DLM bridge names) | `name-layer.ts`, `lib/city/names.ts`, `map-overlay.ts` |
