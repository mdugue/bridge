...
| Ground colour | land-cover class → the one palette, painted on the GPU into an sRGB, mipmapped, anisotropy-16 splat (the fine level at the class raster's edge, the coarse one at half, each texel the mean of its block — `TERRAIN_LEVELS` `splatScale`); the class PNG is decoded byte-exact by `lib/city/png-raster.ts`, never by the browser (WebKit colour-managed and dithered the ids into speckles), and held once per file for both levels (as the NDVI is) | Basis-DLM | `lib/city/landcover.ts`, `landcover-splat.ts`, `terrain-layer.ts` |
