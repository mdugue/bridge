...
| Playground | OSM outline → a 8 cm soft-floor slab (dusty rose) on the ground, skirted 0.4 m; the mapped equipment only (swing, slide, climbing dome, springy, seesaw, roundabout, playhouse, sandpit) stands on it; a sandpit area a sand slab 12 cm above | OSM | `furniture-layer.ts` (`addSlab`), `lib/city/furniture.ts` |
