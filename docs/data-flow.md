...
flowchart LR
  ...
  DGM ==>|"error-bounded TIN (native 1 m)"| TER
...
|---|---|---|---|
| **Terrain ground** | DGM1 GeoTIFF → build-time error-bounded TIN of the native 1 m grid (`.tin-15cm` primary, `.tin-25cm` neighbours) | nothing burned in; the heightfield (`.u16.gz`, + OSM wall conflation) remains the fallback for a tile without a TIN | baked by `scripts/prepare-data.ts` (`lib/city/heightfield.ts`; TIN: `scripts/bake-terrain-tin.ts`, `lib/city/terrain-tin.ts`); `terrain-layer.ts`, `lib/city/terrain-geometry.ts`, `lib/city/terrain-conflate.ts` |
...
| **Trees & hedges** | Basis-DLM rows **+** DOM1−DGM1 canopy **+** LSC crown peaks outside the mask (primary tile, thinned against the cadastre) | DLM class raster *(gates)* · DOP NDVI (crown colour) | `vegetation-layer.ts`; baked by `extract-dlm.sh` + `extract-canopy.sh` + `extract-ndvi.sh` |
| **OSM hedges** | OSM `barrier=hedge` lines | LSC (measured height, primary tile) · DGM1 (ground-clamp); tag / 1.5 m where no LAZ. The bake's laser-scan-only hedges and shrubs are not shipped (🗃️ in the ledger) | `low-vegetation-layer.ts`; baked by `extract-lowveg.sh` |
...
| **Retaining walls** | OSM `barrier=retaining_wall/city_wall/wall` + `height` (local `.pbf`) | DGM1 (ribbon snapped to the measured step of the TIN; on a heightfield tile the terrain is conflated to a step instead) — *no DGM/DOM/LiDAR product has the wall as a vertical face* | `wall-layer.ts`, `lib/city/terrain-conflate.ts`, `lib/city/wall-snap.ts`; baked by `scripts/extract-walls.sh` |
