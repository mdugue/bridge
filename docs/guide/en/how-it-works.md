...
| **Street names** | From the air, the names of streets, squares and bridges lettered on the ground like a map; on foot, the name of the street you are on in a small caption at the top | **OpenStreetMap** (`name` on the ways, named squares); bridge names from the Basis-DLM | Real names; where the lettering sits along a street is chosen by the project (its straightest stretch), and the minor streets' names give way to the main roads from high up. Street-name *signs* are not shown (they are hardly mapped) |
