...
| **Stairs** | Flights of steps such as the one beside the Italienisches Dörfchen, as individual steps | OpenStreetMap (`highway=steps`: position, width, step count); the heights at the foot and the head from the terrain model | Real position and height; step count as tagged or estimated from the height (16 cm a step) |
