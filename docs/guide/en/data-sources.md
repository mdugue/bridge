...
| **DOM1** | Surface model, 1 m grid (terrain *plus* everything standing on it) | GeoSN | Tree heights (surface minus terrain); a bridge's roadway height and what stands above it: trusses, pylons, arches |
...
| **Weaknesses** | Nothing below the eaves: no windows, doors, materials or colours. The building-function attribute is "unspecified" for about 86 % of buildings and the storey count is filled in for only a few percent, so the viewer derives storey bands from the measured height. Bridges are in the tiles as flat, 1 m thick slabs at one height (function `53001_1800`, "structure in the traffic area") — the viewer leaves them out and builds bridges from the landscape and surface models. |
