...
| **Laser scan (LAZ)** 🧪 | hedges + shrubs 0.5–3 m and trees outside the canopy mask (`?veg=`) | **OSM only**: mapped hedges at their `height` tag (or 1.5 m) + `natural=shrub` nodes; no extra trees | sparser points (< 4 /m²): the 0.5 m rasters get holes — bin at 1 m and raise `MIN_AREA_M2` | classified low vegetation (ASPRS 3/4) would replace the NDVI + intensity cue outright |
